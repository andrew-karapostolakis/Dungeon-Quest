...
ctypes.windll.kernel32.SetConsoleTitleW("Dungeon Quest")
...
def start():
	...
	action = input("Press Enter to begin, or type e to exit. ").lower()
	# enter first room
	if action != "e":
		room1()
...
def room2():
	...
	while loop:
		...
		if action in ('look around', 'look', 'survey', 'look at room'):
			...
			# enter first room
			room1()
		elif action in ("wooden door", "door", "door with handle", "wooden door with handle", "handle"):
			loop = False
			# enter third room
			room3()
...
def room4():
	...
	while not (action in ("door", "go through door", "open door") and troll4 == False):
		# ask user for action
		action = input("What do you do? ").lower()
		if action in ("fight troll", "kill troll", "beat up troll", "punch troll in the face", "fight", "attack") and troll4 == True:
			while troll4 == True:
				print("The troll has ", int(troll['health']), " health. You have ", int(player['health']), " health. What do you do?")
				print("ATTACK   FLEE")
				action = input().lower()
				if action == ("attack"):
					# torch deals 2 damage (only weapon available at this point)
					print("\nYou attack the troll and deal ", int((player['attack'] * 2) / troll['defence']), " damage.\n")
					...
					if troll['health'] <= 0:
						...
						print("The troll counter-attacks and deals ", int((troll['attack'] * 5) / player['defence']), " damage.\n")
			...
			print("You can either go through the door or the dark doorway.")
		elif action in ("doorway", "dark doorway"):
			room2()
		elif action in ("stats", "health"):
			print(player)
...
def room5():
	...
	action = input("Thank you for playing Dungeon Quest. To play again, press Enter. To quit, enter Quit.").lower()
	# restart
	if action != "quit":
		start()
